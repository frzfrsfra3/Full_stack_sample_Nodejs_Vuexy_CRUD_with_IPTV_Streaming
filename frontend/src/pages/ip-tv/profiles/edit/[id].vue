...
// Methods
ipTvProfilesStore.fetchProfileByID(route.params.id).then((res) => {
...
const onSubmit = () => {
  refForm.value.validate().then((res) => {
    if (res.valid) {
      ipTvProfilesStore.updateProfile(profile.value).then((res) => {
  ...
  <VCard>
    <VCardItem>
      <VCardTitle class="mb-5">
        <h2 class="text-h5">{{ $t('Update IPTV profile') }}</h2>
      </VCardTitle>
      <VForm ref="refForm"
             @submit.prevent="() => {}">
        <VRow>
